...
public extension XSD {
	...
	static func ·floatLexicalMap· (
		...
		if let numericalSpecialRep = XSD.numericalSpecialRep(LEX) {
			...
			{ nV = XSD.DecimalNumber(XSD.·noDecimalMap·(noDecimalPtNumeral)) }
			else if let decimalPtNumeral = XSD.decimalPtNumeral(LEX)
			{ nV = XSD.·decimalPtMap·(decimalPtNumeral) }
			else
			{ nV = XSD.·scientificMap·(XSD.scientificNotationNumeral(LEX)!) }
			if nV == 0 {
				guard let first = String(LEX).first
				else
				{ return 0 }
				if first == "-"
				{ return -0.0 }

...
public extension XSD {
	...
	static func ·fractionDigitsCanonicalFragmentMap·(
		...
		let ·lastSignificantDigit·FractionDigitRemainderSeq·f =
			XSD.·lastSignificantDigit·(
				XSD.·FractionDigitRemainderSeq·(f)
			)
		var result: Array<String> = []
		result.reserveCapacity(
			Int(·lastSignificantDigit·FractionDigitRemainderSeq·f)
		)
		for _ in 0...(
			...
			result += CollectionOfOne(
				String(
					describing: XSD.·digit·(
						·fractionDigitSeq·f.next()!
					)
				)
			)
		}
